...
#Intialize Lists and Settings
class NaturalDisasterItem(scrapy.Item):
    Time = scrapy.Field()
    Title = scrapy.Field()
    Description = scrapy.Field()
    Link = scrapy.Field()

class naturalspider(scrapy.Spider):
    name = 'naturalspider'

    custom_settings = {
        'FEED_URI': 'webscraper/output/naturaldisaster.csv',
        'FEED_FORMAT': 'csv',
    ...
    def __init__(self, current_date, days_year, *args, **kwargs):
        ...
        super(naturalspider, self).__init__(*args, **kwargs)
    ...
    #Set page range for spider to scrape
    def start_requests(self):
        for i in range(1, self.scope+1): 
            self.scope = self.scope + 1
            yield scrapy.Request(url=f'https://www.reuters.com/news/archive/tsunami?view=page&page={i}&pageSize=10', callback=self.parse)
    ...
    def parse(self, response):
        ...
        self.scope += 1
        yield scrapy.Request(url=f'https://www.reuters.com/news/archive/tsunami?view=page&page={self.scope}&pageSize=10', callback=self.parse)
    ...
    def parse_article(self, response):
        ...
        time_elements2 = sel.css('div.ArticleHeader-date-Goy3y::text').get()
    ...
    def close (self, reason):
        ...
        df_sorted.to_csv('webscraper/output/naturaldisaster.csv', index=False)
        # Close the Selenium driver when the spider finishes
        self.driver.quit()

    # scrapy crawl naturalspider